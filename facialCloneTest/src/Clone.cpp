...
void main() {\
	...
	if(srcColorBlur.a > 0.) {\
		...
		gl_FragColor = vec4(srcColor, 1.);\
	} else {\
		gl_FragColor = vec4(0.);\
	}\
}";
//Em in the file above you chaged srcColor + offset to just srcColor on line39
...
void Clone::update(ofTexture& src, ofTexture& dst, ofTexture& mask) {
	...
	//maskedBlur(dst, mask, dstBlur);       //if i comment this out it makes the whole screen white, but the mask is still there
